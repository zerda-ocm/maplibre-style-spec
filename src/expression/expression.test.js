...
describe('evaluate expression', () => {
    test('silently falls back to default for nullish values', () => {
        ...
        expect(value.evaluate({ globalState: { x: 5 }, zoom: 10 })).toBe(5);
        expect(console.warn).not.toHaveBeenCalled();
        expect(value.evaluate({ globalState: {}, zoom: 10 })).toBe(42);
        expect(console.warn).not.toHaveBeenCalled();
    });
    test('global state as expression property', () => {
        ...
        vi.spyOn(console, 'warn').mockImplementation(() => { });
        expect(value.evaluate({ globalState: { x: 15 }, zoom: 10 })).toBe(5);
        expect(console.warn).not.toHaveBeenCalled();
    });
    ...
    test('warns for invalid variableAnchorOffsetCollection values', () => {
        ...
        expect(value.kind).toBe('source');
        expect(value.evaluate({}, { properties: { x: 'invalid' } })).toBeNull();
        expect(console.warn).toHaveBeenCalledTimes(1);
        expect(console.warn).toHaveBeenCalledWith('Could not parse variableAnchorOffsetCollection from value \'invalid\'');
        warnMock.mockClear();
        expect(value.evaluate({}, { properties: { x: ['top', [2, 2]] } })).toEqual(new VariableAnchorOffsetCollection(['top', [2, 2]]));
        expect(console.warn).not.toHaveBeenCalled();
    });
});
