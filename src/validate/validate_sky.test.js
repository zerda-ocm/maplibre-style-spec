...
describe('Validate sky', () => {
    it('Should pass when value is undefined', () => {
        const errors = validateSky({ validateSpec: validate, value: undefined, styleSpec: v8, style: {} });
        expect(errors).toHaveLength(0);
    });
    test('Should return error when value is not an object', () => {
        const errors = validateSky({ validateSpec: validate, value: '', styleSpec: v8, style: {} });
        expect(errors).toHaveLength(1);
        expect(errors[0].message).toContain('object');
        expect(errors[0].message).toContain('expected');
    });
    test('Should return error in case of unknown property', () => {
        const errors = validateSky({ validateSpec: validate, value: { a: 1 }, styleSpec: v8, style: {} });
        expect(errors).toHaveLength(1);
        expect(errors[0].message).toContain('a');
        expect(errors[0].message).toContain('unknown');
    });
    test('Should return errors according to spec violations', () => {
        const errors = validateSky({ validateSpec: validate, value: { 'sky-color': 1, 'fog-color': 2, 'horizon-fog-blend': {}, 'fog-ground-blend': 'foo' }, styleSpec: v8, style: {} });
    ...
    test('Should pass if everything is according to spec', () => {
        const errors = validateSky({ validateSpec: validate, value: { 'sky-color': 'red', 'fog-color': '#123456', 'horizon-fog-blend': 1, 'fog-ground-blend': 0 }, styleSpec: v8, style: {} });
        expect(errors).toHaveLength(0);
    });
});
