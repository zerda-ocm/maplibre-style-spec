...
function checkErrorMessage(message, key, expectedType, foundType) {
...
describe('Validate source_raster_dem', () => {
    test('Should pass when value is undefined', () => {
        const errors = validateRasterDEMSource({ validateSpec: validate, value: undefined, styleSpec: v8, style: {} });
        expect(errors).toHaveLength(0);
    });
    test('Should return error when value is not an object', () => {
        const errors = validateRasterDEMSource({ validateSpec: validate, value: '', styleSpec: v8, style: {} });
        expect(errors).toHaveLength(1);
        expect(errors[0].message).toContain('object');
        expect(errors[0].message).toContain('expected');
    });
    test('Should return error in case of unknown property', () => {
        const errors = validateRasterDEMSource({ validateSpec: validate, value: { a: 1 }, styleSpec: v8, style: {} });
        expect(errors).toHaveLength(1);
        expect(errors[0].message).toContain('a');
        expect(errors[0].message).toContain('unknown');
    });
    test('Should return errors according to spec violations', () => {
        const errors = validateRasterDEMSource({ validateSpec: validate, value: { type: 'raster-dem', url: {}, tiles: {}, encoding: 'foo' }, styleSpec: v8, style: {} });
    ...
    test('Should return errors when custom encoding values are set but encoding is "mapbox"', () => {
        const errors = validateRasterDEMSource({ validateSpec: validate, value: { type: 'raster-dem', encoding: 'mapbox', 'redFactor': 1.0, 'greenFactor': 1.0, 'blueFactor': 1.0, 'baseShift': 1.0 }, styleSpec: v8, style: {} });
    ...
    test('Should return errors when custom encoding values are set but encoding is "terrarium"', () => {
        const errors = validateRasterDEMSource({ validateSpec: validate, value: { type: 'raster-dem', encoding: 'terrarium', 'redFactor': 1.0, 'greenFactor': 1.0, 'blueFactor': 1.0, 'baseShift': 1.0 }, styleSpec: v8, style: {} });
        expect(errors).toHaveLength(4);
    ...
    test('Should pass when custom encoding values are set and encoding is "custom"', () => {
        const errors = validateRasterDEMSource({ validateSpec: validate, value: { type: 'raster-dem', encoding: 'custom', 'redFactor': 1.0, 'greenFactor': 1.0, 'blueFactor': 1.0, 'baseShift': 1.0 }, styleSpec: v8, style: {} });
        expect(errors).toHaveLength(0);
    });
    test('Should pass if everything is according to spec', () => {
        const errors = validateRasterDEMSource({ validateSpec: validate, value: { type: 'raster-dem' }, styleSpec: v8, style: {} });
        expect(errors).toHaveLength(0);
    });
});
