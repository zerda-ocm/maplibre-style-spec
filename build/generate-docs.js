 ...
 */
function topicElement(key, value) {
 ...
 */
function exampleToMarkdown(key, example) {
    return codeBlockMarkdown(`"${key}": ${formatJSON(example)}`);
}
function codeBlockMarkdown(code, language = 'json') {
    return `\`\`\`${language}\n${code}\n\`\`\`\n`;
}
 ...
 */
function sdkSupportToMarkdown(support) {
    let markdown = '\n';
 ...
 */
function expressionSyntaxToMarkdown(key, syntax) {
    var _a;
    let markdown = '\nSyntax:\n';
    ...
    for (const parameter of (_a = syntax.parameters) !== null && _a !== void 0 ? _a : []) {
 ...
 */
function requiresToMarkdown(requires) {
    let markdown = '';
 ...
 */
function typeToMarkdownLink(type) {
 ...
 * @returns the markdown string
 */
function convertPropertyToMarkdown(key, value, keyPrefix = '##', paintLayoutText = '') {
    var _a;
    ...
    if ((_a = value.expression) === null || _a === void 0 ? void 0 : _a.interpolated) {
    ...
    if (value.example !== undefined) {
        markdown += exampleToMarkdown(key, value.example);
    }
...
function createLayersContent() {
    ...
    for (const [key, value] of Object.entries(v8.layer)) {
        content += convertPropertyToMarkdown(key, value, '###');
    }
    for (const layoutKey of Object.keys(v8).filter(key => key.startsWith('layout_'))) {
        ...
        for (const [key, value] of Object.entries(v8[layoutKey])) {
            content += convertPropertyToMarkdown(key, value, '###', 'Layout');
        }
        for (const [key, value] of Object.entries(v8[`paint_${layerName}`])) {
            content += convertPropertyToMarkdown(key, value, '###', 'Paint');
        }
    }
    ...
    for (const [key, value] of Object.entries(v8.$root)) {
        ...
        if (value.type in v8) {
            for (const [subKey, subValue] of Object.entries(v8[value.type])) {
                content += convertPropertyToMarkdown(subKey, subValue);
            }
        }
        if (value['sdk-support']) {
            content += '\n---\n';
